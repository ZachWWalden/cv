...
\begin{multicols}{2}
		\begin{itemize}[label=$\bullet$,itemsep=-.35ex]
			\item \large{Filler}
			\item \large{Excellent software debugging skills}
			\item \large{Filler}
...
\section{Projects}
	...
	\subsection{RGB Laser Scanner Controller \hfill \normalfont{\small{\textsl{Oct. 2020 - Apr. 2021}}}}
		...
		\end{itemize}
	\subsection{Stock Ticker \hfill \normalfont{\small{\textsl{Dec. 2022 - Oct. 2023}}}}
		Wrote a concurrent stock ticker program in C++ that pulls information about a stock from an api, processes it, and then renders it to a 64x32 RGB LED matrix.
		\begin{itemize}[label=--,itemsep=-.35ex]
			\item \large{Libcurl was used to make http requests.}
			\item \large{Used the jsoncpp C++ library to parse through stock information and program configuration}
			\item \large{Used a 2 thread, shared memory, producer-consumer architecture}
			\item \large{}
			\item \large{}
		\end{itemize}
