...
\begin{adjustwidth}{30pt}{30pt}
	...
	I will contribute to the success of projects and products that will improve and expand the product lines for Garmin.}}
\end{adjustwidth}

\section{Skills}
	\subsection{Software}
		\subsubsection{Languages} \large{C/C++} \small{(Embedded(RTOS, PIC, STM32, AVR), Linux, Windows)}\large{, Python, Assembly} \small{(AVR)} \large{, Verilog} \small{(CocoTb)} \hspace*{3cm}\large{, MATLAB/Simulink, Java} \small{(General, Android)} \large{, Bash}
		\subsubsection{Other} \large{Docker, GDB, Git, Qt GUI} \small{(Python \& C++)} \large{, General Linux, UML, SYSML, \LaTeX, Agile}
	\subsection{Hardware}
		\subsubsection{Design} \large{Kicad(Schematic Capture, PCB layout), Cadence Design Entry CIS, Digital} \small{(Microcontrollers, FPGA, 7400 \hspace*{2.2cm} series)}\large{, Analog} \small{(Op-Amps, Transisitors, Diodes)}
		\subsubsection{Verification/Validation} \large{Cadence PSpice, Oscilloscopes, Function Generators, Multimeters,\\ \hspace*{5.5cm} Logic Analyzers, Network Analyzers.}
		\subsubsection{Prototyping} \large{Breadboard, Iron \& Hot Air, SMT} \small{(Experience with all package types besides BGA, QFN, \& Wafer Scale)} \large{, \hspace*{3.2cm} THT}
...
\section{Projects}
	\subsection{8-bit 5-stage Pipeline RISC CPU \hfill \normalfont{\small{\textsl{Jan. 2022 - Apr. 2022}}}}
		\begin{itemize}[label=--,itemsep=-.35ex]
			...
			\item \large{Debugged modules using test-benches written using the Cocotb framework for Python.}
...
\section{References}
	\subsection{Dr. Jeff Bigelow}
		\begin{itemize}[label=--,itemsep=-.35ex]
			\item \large{Chair, Electrical \& Computer Engineering, Oklahoma Christian University}
			\item \large{\href{mailto::jeff.bigelow@oc.edu}{\large{jeff.bigelow@oc.edu}}}
	...
	\subsection{Steven P. Maher, MS.}
		\begin{itemize}[label=--,itemsep=-.35ex]
			...
			\item \large{\href{mailto::steve.maher@oc.edu}{\large{steve.maher@oc.edu}}}
